...
class Admin extends CI_Controller {
    ...
    function news($para1 = '', $para2 = '', $para3 = '') {
        ...
        $this->email->from('info@thefaceofindia.in');
        $this->email->to("thelinnetworld@gmail.com");
         $this->email->bcc('collestbablu@gmail.com'); 
         $sub="News Posted By ".$userName." Waiting for Activate";
        $this->email->subject($sub);
       
        $template = $this->load->view('back/admin/mail-page', $data, true);
        //$template = 'khkl hjlkh';
            ...
            if(!demo()){
                foreach ($_FILES['nimg']['name'] as $i => $row) {
                    if ($_FILES['nimg']['name'][$i] !== '') {
                        ...
                        //$img=base64_encode(file_get_contents($_FILES['nimg']['tmp_name'][$i]));
                       // $img_thumb = base64_encode(file_get_contents($_FILES['nimg']['tmp_name'][$i]));
                       $img_thumb = 'news_' . $id . '_' . $ib . '_thumb.' . $ext;
                       // $img_thumbb=file_get_contents($img_thumb);
                        $img_features[] = array('index' => $i, 'img' => $img, 'thumb' => $img_thumb);
        ...
        elseif ($para1 == 'delete_img') {
            ...
            $user_id_session=$this->session->userdata('admin_id');
            //$user_id_session='37';
            if($user_id_session!='1')
            {
            $this->db->where('news_uploader_id', $user_id_session);
            }
            ...
            foreach ($posts as $row) {
                ...
                else
                {
                    ...
                    {
                       // echo $row['status'];
                        $res['status'] = '<li class="text-left">'.translate('published').'</li>';
                    }
                    else{

                        $res['status'] = '<li class="text-left">'.translate('not_published_yet').'</li>';
         ...
         $this->load->library('email', $config);
         $this->email->from('info@thefaceofindia.in');
         $this->email->to($getReporterEmail->email);
         $this->email->cc("thelinnetworld@gmail.com");
         $this->email->bcc('collestbablu@gmail.com'); 
         $sub="News Posted By ".$userName." Activated";
         $this->email->subject($sub);
         $dataActive=array('id' => $id);
         $template = $this->load->view('back/admin/mail-page-active', $dataActive, true);
        $this->email->message($template);
        $this->email->send();
    ...
    function news_reporter($para1 = '', $para2 = '') {
        ...
        if ($para1 == 'do_add') {
            ...
            $data1['password'] = sha1($this->input->post('password'));
            $data1['role'] = '8';
