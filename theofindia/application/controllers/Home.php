...
function archive_ajax_news_list($para1 = '') {
    ...
    function registration($para1 = "", $para2 = "") {
                    ...
                    if ($this->Crud_model->get_settings_value('third_party_settings', 'captcha_status', 'value') == 'ok') {
                        ...
                        if ($response['success']) {
                            ...
                            if ($this->input->post('password1') == $this->input->post('password2')) {
								...
								echo $this->input->post('is_reporter');die;
                                $this->db->insert('user', $data);
								if (!empty($this->input->post('is_reporter'))) {
							$password = $this->input->post('password1');
                            $data1['password'] = sha1($password);
							$data1['name'] = $this->input->post('firstname');
                        	$data1['email'] = $this->input->post('email');
                        	$data1['present_address'] = $this->input->post('address1');
                        	$data1['phone'] = $this->input->post('phone');
							$data2['password'] = sha1($password);
							$data2['name'] = $this->input->post('firstname');
                        	$data2['email'] = $this->input->post('email');
                        	$data2['present_address'] = $this->input->post('address1');
                        	$data2['phone'] = $this->input->post('phone');
							$data2['role'] = '8';
                        	$this->db->insert('news_reporter', $data1);
                        ...
                        if ($this->input->post('password1') == $this->input->post('password2')) {
							...
							$data2['name'] = $this->input->post('firstname');
                        	$data2['email'] = $this->input->post('email');
                        	$data2['address'] = $this->input->post('address1');
                        	$data2['phone'] = $this->input->post('phone');
							$data2['role'] = '8';
							$this->db->insert('news_reporter', $data1);
							$this->db->insert('admin', $data2);
     ...
     /* FUNCTION: reporter Registration */

    function reporter_registration($para1 = '', $para2 = '') {
      $safe = 'yes';
        $char = '';
        foreach ($_POST as $k => $row) {
            if (preg_match('/[\'^":()}{#~><>|=¬]/', $row, $match)) {
        ...
        if ($para1 == "add_info") {
            ...
            //$this->load->view('front/header/header-1.php');
            //$this->load->view('front/user/registration/reporter-registration', $page_data);
           $this->load->view('front/registration', $page_data);
        }
